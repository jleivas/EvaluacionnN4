    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (usuario == null || usuario.isBlank() || password == null || password.isBlank()) {
            ...
            if (admin != null && admin.getPassword().equals(password)) {
                ...
                request.getRequestDispatcher("/PanelAdmin/AdminPanel.jsp").forward(request, response);

            } else if (prof != null && prof.getPassword().equals(password)) {
                int id = prof.getIdProf();
                session.setAttribute("loggedIn", true);
                session.setAttribute("id", id);
                request.getRequestDispatcher("/PanelProf/ProfPanel.jsp").forward(request, response);
            } else if (cliente != null && cliente.getPassword().equals(password)) {
                int id = cliente.getIdCliente();
                session.setAttribute("loggedIn", true);
                session.setAttribute("id", id);
                request.getRequestDispatcher("/PanelCliente/ClientePanel.jsp").forward(request, response);
            } else {
